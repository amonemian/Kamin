...
class XORConfig:
	...
	def fitness_fn(self, genome):
		...
		for input, target in zip(self.inputs, self.targets):  # 4 training examples
			...
			pred = phenotype(input) #calling the FeedForwardNet (which is torch Module and calling it eventually causes forward to be called)
...
print("===========")
XORConfig.inputs = list(map(lambda s: autograd.Variable(torch.Tensor([s])), [
		[0, 0],
		[0, 1],
		[1, 0],
		[1, 1]
...
XORConfig.targets = list(map(lambda s: autograd.Variable(torch.Tensor([s])), [
	[0],
	[1],
	[1],
	[0]
