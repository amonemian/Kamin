...
class ConnectionGene:
    ...
    def set_rand_weight(self):
        ...
        """
        #self.weight = torch.Tensor(torch.normal(torch.arange(0, 1), dtype=torch.float)).to(device)
        self.weight = torch.Tensor(torch.normal(0., 1., (1,), dtype=torch.float)).to(device) #BYME
        #print(self.weight)
